...
def index():
    ...
    ai_available = os.environ.get("OPENAI_API_KEY") and os.environ.get("OPENAI_API_KEY") != "your-openai-api-key-here"

    if request.method == "POST":
        question = request.form.get("question")
        
        if not ai_available:
            session['error'] = "⚠️ AI-funktionalitet er ikke tilgængelig. Tilføj din OpenAI API key til .env filen for fuld CRM funktionalitet."
...
def crm_stats():
    ...
    try:
        ...
        # Customer stats
        customer_stats = run_query("SELECT COUNT(*) as total, COUNT(CASE WHEN status = 'Active' THEN 1 END) as active FROM customers")
        stats['customers'] = customer_stats[0] if customer_stats else {'total': 0, 'active': 0}
        
        # Deal stats
        deal_stats = run_query("SELECT COUNT(*) as total, SUM(value) as total_value, AVG(probability) as avg_probability FROM deals WHERE stage NOT IN ('Closed Won', 'Closed Lost')")
        stats['deals'] = deal_stats[0] if deal_stats else {'total': 0, 'total_value': 0, 'avg_probability': 0}
        
        # Project stats
        project_stats = run_query("SELECT COUNT(*) as total, COUNT(CASE WHEN status = 'In Progress' THEN 1 END) as active FROM projects")
        stats['projects'] = project_stats[0] if project_stats else {'total': 0, 'active': 0}
        
        # Consultant stats
        consultant_stats = run_query("SELECT COUNT(*) as total, AVG(hourly_rate) as avg_rate FROM consultants WHERE status = 'Active'")
        stats['consultants'] = consultant_stats[0] if consultant_stats else {'total': 0, 'avg_rate': 0}
...
def customers():
    ...
    try:
        ...
        stats = {
            ...
            'avg_projects': sum([c['project_count'] for c in customers_data]) / len(customers_data) if customers_data else 0
        }
...
def deals():
    ...
    try:
        ...
        stats = {
            ...
            'pipeline_value': sum([d['value'] for d in deals_data if d['stage'] not in ['Closed Won', 'Closed Lost']])
        }
...
def projects():
    ...
    try:
        ...
        stats = {
            ...
            'avg_budget': sum([p['budget'] for p in projects_data]) / len(projects_data) if projects_data else 0
        }
...
def consultants():
    ...
    try:
        ...
        return render_template("consultants.html", consultants=consultants_data, stats=stats)
        
    except Exception as e:
        return render_template("consultants.html", consultants=[], stats={}, error=str(e))
...
def activities():
    ...
    try:
        ...
        stats = {
            ...
            'this_week': len([a for a in activities_data if a['activity_date'] and a['activity_date'] >= week_ago.strftime('%Y-%m-%d')]),
            'meetings_calls': len([a for a in activities_data if a['type'] in ['Meeting', 'Call']]),
            'follow_ups': len([a for a in activities_data if a['outcome'] == 'Follow-up needed'])
        }
